...
export default function RouteCard({ route, onToggleDetails, isSelected }) {
  const label = route.label && labelText[route.label];
  ...
  const totalHours = (route.total_duration_minutes / 60).toFixed(1);

  return (
    <div className={`route-card ${isSelected ? "route-card-selected" : ""}`}>
      <div className="route-card-main">
        <div className="route-card-left">
          ...
          <p className="route-card-meta">
            {totalHours} ч · {transfersText}
          </p>
          <ul className="segment-list">
            {route.segments.map((s, idx) => (
              <li key={idx} className="segment-item">
                <span className="segment-type">{transportEmoji[s.type]}</span>
